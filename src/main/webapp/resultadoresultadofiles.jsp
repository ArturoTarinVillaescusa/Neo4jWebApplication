...
<body onload=' location.href="#map"' >
	...
	<h2 align="center">Map of the measurements</h2>
	...
	<div>
		...
		<a name="annotation"></a> 
		<input action="action" type="button" style="background-image:url('images/back.png');width:45px;height:45px" onclick="location.href = 'indice.jsp#/clients'" />
		<h2 align="center"><a href="#map">Map of the measurements</a> Measurements chart</h2>
	...
	<script>
		...
		function loadNodeLocations() {
			...
			for (var i = 1; i < numRows; i++) {
				...
				latitud=tableBody.getElementsByTagName("tr")[i].getElementsByTagName("td")[10].childNodes[0].data;
				longitud=tableBody.getElementsByTagName("tr")[i].getElementsByTagName("td")[8].childNodes[0].data;
				// LLS = single laterolog
				lls=tableBody.getElementsByTagName("tr")[i].getElementsByTagName("td")[0].childNodes[0].data;
				// MSFL = micro spherically focused log
				msfl=tableBody.getElementsByTagName("tr")[i].getElementsByTagName("td")[2].childNodes[0].data;
				// GR = gamma ray index
				gr=tableBody.getElementsByTagName("tr")[i].getElementsByTagName("td")[4].childNodes[0].data;
				file=tableBody.getElementsByTagName("tr")[i].getElementsByTagName("td")[5].childNodes[0].data;
				// RHOB = bulk density
				rhob=tableBody.getElementsByTagName("tr")[i].getElementsByTagName("td")[7].childNodes[0].data;
				// DEPTH = we find it to be depth in meters
				depth=tableBody.getElementsByTagName("tr")[i].getElementsByTagName("td")[9].childNodes[0].data;
				// NPHI = neutron porosity
				nphi=tableBody.getElementsByTagName("tr")[i].getElementsByTagName("td")[12].childNodes[0].data;
				// IDL = deep-induction measurement
				idl=tableBody.getElementsByTagName("tr")[i].getElementsByTagName("td")[14].childNodes[0].data;
				
				locations.push(['<h4>File: '+file.substring(file.lastIndexOf('\\')+1)+'<br/> Depth: '+depth+'<br/> Gamma ray index: '+gr+
				                '<br/> Single laterolog: '+lls+'<br/> Micro spherically focused log: '+msfl+'<br/> Bulk density: '+rhob+'<br/> Neutron porosity: '+nphi+'<br/> Deep-induction measurement: '+idl+'<br/> Latitude: '+latitud+', Longitude: '+longitud+'</h4>', latitud, longitud, depth]);
			}
		}
