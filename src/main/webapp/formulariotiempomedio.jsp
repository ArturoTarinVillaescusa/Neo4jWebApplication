<jsp:root xmlns:jsp="http://java.sun.com/JSP/Page" xmlns="http://www.w3.org/1999/xhtml" version="2.0"
	...
	<head> 
		...
		<script>
			...
			function outputUpdate(limtiempomedio) {
				document.querySelector('#limtiempomedio').value = limtiempomedio;
			}
			...
			function crea_columna(vehiculo, mean_time_trip_for_this_vehicle) {
				...
				campoPantalla.setAttribute("value", "formulariotiempomedio");
				...
				campoLimViaje.setAttribute("value", document.getElementById('limtiempomedio').value);
				...
				texto1.setAttribute('value', mean_time_trip_for_this_vehicle+' hours/trip');
				texto1.setAttribute('readonly', 'true');

				var color_background='lightgreen';
				if (mean_time_trip_for_this_vehicle > 8)
					color_background='yellow';
				if (mean_time_trip_for_this_vehicle > 10)
				...
				columna.title = 'It\'s vehicle mean was ' + mean_time_trip_for_this_vehicle+' hours per trip';
			...
			function dibujarTabla (tablaDatos) {
				...
				var num_trips_for_this_vehicle=0;
				var mean_time_trip_for_this_vehicle = 0;
				var vehiculo;
				var fecha;
				var delta_t;
				var vehiculo_temporal;
				var fecha_temporal;
				var fecha_temporal_1;
				...
				for (var x in tablaDatos) {
					...
					do {
						...
						if (tablaDatos[j-1][1].substring(0,10) == tablaDatos[i][1].substring(0,10)) {
							fecha_temporal=new Date(tablaDatos[j-1][1]);
						}
						else {
							if (j > i)
								fecha_temporal=new Date(tablaDatos[j-2][1]);
						}
						
						delta_t_temporal=parseInt(tablaDatos[j-1][2]);

						// If the vehicle is still the same and this is an end of the trip
						// then obtain the duration and include it in the calculation of the mean
						// time of trip for this vehicle
						if ( vehiculo.valueOf() == vehiculo_temporal.valueOf()) {
							if ( delta_t_temporal.valueOf() > 300 ) {
								duration = fecha_temporal - fecha;
								// Convert to hours
								duration = duration / 1000 / 60 / 60;
								if (24 > duration) {
									if (duration > 0) {
							// alert('vehiculo '+vehiculo + 'vehiculo_temporal '+ vehiculo_temporal+' num_trips_for_this_vehicle '+num_trips_for_this_vehicle+'fecha '+fecha+',fecha_temporal '+fecha_temporal+ ',duration '+duration+ ',durations '+durations+' i '+i+' j '+j);								
										num_trips_for_this_vehicle++;
						...
						//alert('i '+i+', j '+j);
						j++;
					} while ((vehiculo.trim() == vehiculo_temporal.trim()));
					
					mean_time_trip_for_this_vehicle = Math.round(durations / num_trips_for_this_vehicle, 1);
				  //  alert('fuera del do. durations dentro del bucle '+durations + ' para '+ vehiculo+'. Hemos cambiado a '+vehiculo_temporal+
				   // 		'mean_time_trip_for_this_vehicle '+mean_time_trip_for_this_vehicle+' num_trips_for_this_vehicle '+num_trips_for_this_vehicle+' i '+i+' j '+j);
					...
					if (mean_time_trip_for_this_vehicle > document.getElementById('limtiempomedio').value) {
						row_tabla_pro.appendChild(crea_columna(vehiculo, mean_time_trip_for_this_vehicle));
						linea++;
					}
				    
				    i = j;
				    num_trips_for_this_vehicle=0;
				    duration=0;
				    durations=0;
				}
				alert('solucionado');
				alert('duration '+duration);
				alert('durations '+durations);
		...
		<table>
			  <tr>
				<td><input name="cypher" id="cypher-in" type="hidden" value="MATCH (v)-[r:HA_ESTADO_EN]->(p) WITH v.vehiculoID as vehiculo, r.fecha as fecha, ToFloat(r.delta_t) as delta_t WHERE delta_t = 0 OR delta_t > 300 RETURN vehiculo, fecha, delta_t order by vehiculo, fecha" /></td>
					<label for="fader">Vehicles with mean time of trips bigger than: </label>
					<output for="fader" id="limtiempomedio">7</output>
